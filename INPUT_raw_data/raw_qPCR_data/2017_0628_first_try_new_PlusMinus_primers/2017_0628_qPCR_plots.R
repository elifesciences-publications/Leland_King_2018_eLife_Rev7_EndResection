...
if ( file.exists("2017_0628_all_raw_data.RData") ) {
  load("2017_0628_all_raw_data.RData")
} else {
  # use the helper function qpcr_import() from above to import the raw excel data:
  qp <- qpcr_import("RawData/","2017_0628_well_info.xlsx")
  # save this as an .RData file to speed things up next time around:
  save(qp, file = "2017_0628_all_raw_data.RData" )
}
...
# amplification curves for std curves only:
qp$cycle %>%
  filter(is.std) %>%
  mutate(uniq.std.curves = interaction(primers,digest,sep=" ")) %>%
...
# all amplifications for comparision with -DNA wells:
qp$cycle %>%
  ggplot(aes(x=cycle, y=RFU, group=well, color=is.noDNA)) + 
  geom_line(size=0.4) +
  facet_wrap(~primers) + 
  scale_color_manual(name="no DNA controls",values = c("grey75","red")) + 
  scale_y_continuous(name="RFU") +
  scale_x_continuous(name="Cycle") + 
  theme_bw()
ggsave(filename = file.path("Rplots","validate_check_minusDNA_controls.pdf"),
       device = cairo_pdf(),
       scale = 1, width = 14, height = 10, units = "cm")
...
# All calculated concentrations for every relavent well
tmp.exp.conc <- 2.5
qp$summ.calc.conc %>%
  filter(conc == tmp.exp.conc, primers != "Ncb2_v2") %>% # get rid of everything except the ApoI protection exp
  arrange(strain,primers,time,digest) %>% 
  mutate(bar.groups = str_c(strain," ",primers," ",time,"min"," ",digest)) %>% 
  ggplot(aes(x=bar.groups, y=calc.conc, fill=factor(rep))) +
  geom_bar(stat="identity",position="dodge",color="black") + 
  scale_fill_manual(values = c("grey","grey","grey"),guide=FALSE) + 
  scale_y_continuous(name="Calculated Conc. [ng/uL]") + 
  theme_bw() + 
...
qp$summ.calc.conc %>% 
  filter(primers == "m3023", digest == "ApoI", time == 180, conc == 2.5) %>%
  select(plate:is.empty,Cq,calc.conc)
tmp.excluded.wells <- c("B04","F05")

# All calculated concentrations for every relavent well (mark excluded ones though):
qp$summ.calc.conc %>%
  filter(conc == tmp.exp.conc, digest != "none", primers != "Ncb2_v2") %>% # get rid of everything except the ApoI protection exp
  ...
  ggplot(aes(x=bar.groups, y=calc.conc, fill=is.excluded)) +
...
ggsave(filename = file.path("Rplots","bar_plot_all_calculated_concentrations.pdf"),
       device = cairo_pdf(),
       scale = 1, width = 18, height = 14, units = "cm")
...
tmp.exp.conc <- 2.5
qp$summ.calc.conc %>%
  filter(conc == tmp.exp.conc, digest != "none", primers != "Ncb2_v2") %>%
  mutate(is.excluded = if_else(well %in% tmp.excluded.wells, TRUE, FALSE),
